...
### Step 0: Version Stamp Diff (run this first, before reading anything else)

```
grep -rn "Generated by A-Team on" {team}/CLAUDE.md {team}/AGENTS.md 2>/dev/null
bash scripts/validate-team.sh {team}
```

Record the result before any other assessment. Three outcomes:

- **No stamp anywhere** — the team predates stamping. Everything below is assessed against today's rules, but classify defects as *un-retrofitted history*, not as live generator bugs. Do not propose generator changes on this evidence alone.
- **Stamp present, single platform tree** — note the date; defects newer than it are live.
- **Multiple platform trees with missing or mismatched stamps** — classify as DRIFT and report it ahead of every other finding. Divergent dates mean the trees are out of sync, and the divergence is usually larger than the stamp: diff the rule and agent counts per tree too. This is `rules/dual-platform-parity.md` M3/M4; the rule exists because `presentation-studio` drifted (Codex tree retrofitted, Claude tree stale, no stamp) undetected for ~2 months.

No structural change may be proposed until this diff is recorded.
...
- `rules/reviewer-mandate.md`: Verify process reviewer existence in assessed teams. Path-scoped to `teams/**` — it auto-loads once you read a file in the target team, which Step 1 does.
- `rules/dual-platform-parity.md`: M1–M5, the basis for Step 0. Path-scoped to the platform trees.
